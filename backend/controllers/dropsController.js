...
const getLatestDrop = async (req, res) => {
  try {
    ...
    // Get the latest drop
    console.log('Debug - Fetching latest drop for user:', userId.toLowerCase());
    ...
    console.log('Debug - Latest drop query result:', { data: latestDrop, error: dropError });
    ...
    // If no drop exists, generate one
    if (!latestDrop) {
      ...
      console.log('Debug - Generated new drop:', newDrop);
      ...
      if (!newDrop) {
        ...
        console.log('Debug - Returning empty drop:', response);
        return res.json(response);
      }

      // Store the new drop
      console.log('Debug - Storing new drop');
      ...
      console.log('Debug - Returning new drop:', response);
      return res.json(response);
    }

    // Get previous week's drop for comparison
    const prevWeekStart = subWeeks(new Date(latestDrop.week_start), 1);
    console.log('Debug - Fetching previous week drop');
    ...
    console.log('Debug - Returning existing drop:', response);
    res.json(response);
  } catch (error) {
    console.error('Server error:', error);
    res.status(500).json({ error: error.message });
